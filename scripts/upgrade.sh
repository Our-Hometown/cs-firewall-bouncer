...
upgrade_bin() {
    rm "${BIN_PATH_INSTALLED}" || (echo "crowdsec-firewall-bouncer-oht is not installed, exiting." && exit 1)
...
log_err() {
    msg=$1
    date=$(date +%x:%X)
    echo -e "${RED}ERR${NC}[${date}] crowdsec-firewall-bouncer-oht: ${msg}" 1>&2
}
...
echo "crowdsec-firewall-bouncer-oht upgraded successfully."